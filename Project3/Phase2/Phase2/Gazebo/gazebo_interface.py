...
def move(child_t, c2c, step_size, delta_theta):
    # print(f"CHOLD_T: {child_t}")
    ori = child_t[2]
...
def move_fwd(child_t, c2c, step_size):
    return move(child_t, c2c, step_size, 0)

def move_fwd_ccw30(child_t, c2c, step_size):
    return move(child_t, c2c, step_size, 30)

def move_fwd_ccw60(child_t, c2c, step_size):
    return move(child_t, c2c, step_size, 60)

def move_fwd_cw30(child_t, c2c, step_size):
    return move(child_t, c2c, step_size, -30)

def move_fwd_cw60(child_t, c2c, step_size):
    return move(child_t, c2c, step_size, -60)
...
#-----------------------------------------------------------
def run_AStar(start_pos, goal_pos, step_size, all_collisions, clearance):
    """
    ...
    # add to dictionary
    open_cost[child_t_disc] = (None, c2c, goal_pos)

    # add to heap
    heapq.heappush(open_list, (ct, info))

    # start loop
    t_fin = compare_against_goal(goal_pos, start_pos, step_size, all_collisions, clearance)

    return t_init, t_fin

#-----------------------------------------------------------
def compare_against_goal(goal_pos, start_pos, step_size, all_collisions, clearance):
    """
    ...
    while open_list:
        ...
        # print(f"CHILD T DISC: {child_t_disc}, CHILD T: {child_t}")

        # extract info from dictionary
        parent_disc_state, c2c, actual_state = open_cost[child_t_disc]
        # actual_state = tuple(map(int, actual_state))

        # print(f"PARENT DISC STATE: {parent_disc_state}, ACTUAL STATE: {actual_state}")

        # Check to see if the start_pos has been reached
        if (actual_state[0] - start_pos[0])**2 + (actual_state[1] - start_pos[1])**2 <= 1.5**2:
        ...
        if child_t_disc not in closed_list:
            ...
            generate_possible_moves(start_pos, parent_disc_state, c2c, actual_state, step_size, all_collisions)

    # return t_fin
#-----------------------------------------------------------
def generate_possible_moves(start_pos, parent_disc_state, c2c, actual_state, step_size, all_collisions):
    """
    ...
    for action in Actions_Set:
        ...
        new_state, new_cost = action(actual_state, c2c, step_size)
        x, y, theta = new_state
        # print(f"ACTUAL STATE: {actual_state}")
        # print(f"NEW SSTATE: {new_state}")
        ...
        # update cost
        ct = new_cost + np.sqrt((x - start_pos[0])**2 + (y - start_pos[1])**2)

        # ct = new_cost + np.sqrt(
        #     (start_pos[0] - actual_state[0])**2 +
        #     (start_pos[1] - actual_state[1])**2 +
        #     theta_equiv_dist(start_pos[2], actual_state[2], step_size)**2
        # )
        ...
        if new_state_disc not in open_cost or open_cost.get(new_state_disc)[1] > new_cost:
            open_cost[new_state_disc] = (actual_state, new_cost, new_state)
            heapq.heappush(open_list, (ct, (new_state_disc, new_state)))

#-----------------------------------------------------------
def theta_equiv_dist(theta1, theta2, step_size):
    d = abs((theta1 - theta2) % 360)
    d = min(d, 360 - d)        # wrap to shortest angle
    return (d / 30.0) * step_size
...
def get_inputs():
    ...
    while True:
        ...
        try:
            ...
            gx, gy, gt = map(int, goal_str.split(","))

            start_pos = (sx, sy, st)
            goal_pos = (gx, gy, gt)

            clearance = input("Choose the clearance around the obstacles (also is the robot's radius):")
            clearance = int(clearance)

            step_size = input("Choose step size (1 to 10):")
            step_size = int(step_size)
            # radius = input("Choose the robot's radius:")
        ...
        if not gt % 30 == 0:
            print("Goal angle is not a multiple of 30")
            continue
        ...
        return start_pos, goal_pos, clearance, step_size
...
def check_collisions(all_collisions, start_pos, goal_pos, order):
    while True:
        ...
        start_pos, goal_pos, clearance, step_size = get_inputs()
...
if __name__ == "__main__":
    ...
    # run algorithm
    t_init, t_fin = run_AStar(start_pos, goal_pos, step_size, all_collisions, clearance)
