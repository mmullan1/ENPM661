...
def grid_index(node):
    """
    Break the 200x400 grid down into 0.5cm segments
    """
    x, y, theta = node
...
def discretize(node):
    """
    Re-discretize the values that get saved to 
    the dictionary are in larger bins to speed up the algorithm
    """
    x, y, theta = node
...
def theta_equiv_dist(theta1, theta2):
    """
    Wrap all angles between 0 and 360 degrees
    """
...
def generate_path(goal_pos, start_pos, final_loc, clearance):
    """
    Once the solution is found, backtrack to find the path that leads to it
    """
...
def draw_obstacle_course(order, clearance, start_pos=None, r_bot=None):
    """
    Construct a 200x400 MatPlotLib graph with the obstacle course for
...
def check_collisions(all_collisions, start_pos, goal_pos, order, clearance_cells, RPM1, RPM2, r_bot):
    """
    Check to see if the start node or goal node is out of bounds or inside an obstacle
...
def write_path(order, points_per_meter=40, filename="astar_path.csv"):
    """
    Write the path to a csv file that can be run via gazebo_interface.py
    """
...
def redistribute_path(order, points_per_meter=40):
    """
    Redistribute a 2D path by arc length. This should make the path take 
    around the same amount of time to execute no matter its size
    """
    order = np.asarray(order, dtype=float)
