...
#-----------------------------------------------------------
def run_W_AStar(start_pos, goal_pos, all_collisions, weight):
    """
    ...
    c2c = 0
    ct = c2c + np.sqrt((start_pos[0]-goal_pos[0])**2 + (start_pos[1]-goal_pos[1])**2)*weight
    open_cost[child_t] = (None, c2c)
    heapq.heappush(open_list, (ct, child_t))

    # start loop
    t_fin = compare_against_goal(goal_pos, start_pos, all_collisions, weight)

    return t_init, t_fin

#-----------------------------------------------------------
def compare_against_goal(goal_pos, start_pos, all_collisions, weight):
    """
    ...
    while open_list:
        ...
        if child_t not in closed_list:
            ...
            generate_possible_moves(c2c, child_t, all_collisions, weight)

    # return t_fin
#-----------------------------------------------------------
def generate_possible_moves(c2c, child_t, all_collisions, weight):
    """
    ...
    for action in Actions_Set:
        ...
        x, y = new_state
        ct = new_cost + np.sqrt((start_pos[0]-x)**2 + (start_pos[1]-y)**2)*weight
...
if __name__ == "__main__":
    ...
    # run algorithm
    t_init, t_fin = run_W_AStar(start_pos, goal_pos, all_collisions, 10)
